...
maxclust=length(Z);  %start with maximal number of clusters (=amount of pixel, so that each Pixel would descirbe one ROI)
N_Clusts=0;
N_Clusts_i=0;
...
while N_Clusts <= N_Clusts_i  
    ...
    N_Clusts=N_Clusts_i;
    maxclust=maxclust-1;  %decrease variable maxclust by one as long as number of clusters that 
    % have a size of app. N_Pi_perROI  is bigger in this round compared to previous round
    
    P=cluster(Z,'maxclust',maxclust); %calculates clusters
    
    ALL_N=nan(1,maxclust);
    for i=1:maxclust %calculates how many pixels are assigned to each cluster
        N=find(P==i);
        ALL_N(i)=length(N);
    end
    
    N_Clusts_i=length(find(ALL_N==N_Pi_perROI))+length(find(ALL_N==N_Pi_perROI-1))...
        +length(find(ALL_N==N_Pi_perROI-2)); % Number of clusters that have as many pixels as described 
        % by N_Pi_perROI (-1, -2)
