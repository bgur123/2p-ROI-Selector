function [A,C,newcenters,CC] = manually_refine_components(Y,A,C,centers,img,sx,options, handles)
...
% fig = figure;
a = imagesc(img,[min(img(:)),max(img(:))]);
a.Parent = handles.ROIaxes; %ADDED BY BG
fig = gcf;
...
for i = 1:size(A,2)
    ...
    currentCoordinates = reshape(A(:,i),options.d1,options.d2);
