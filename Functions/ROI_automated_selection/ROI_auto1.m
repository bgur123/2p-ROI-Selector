function ROI_auto1(timeSeries, expectedROINumber)
...
tau = 8;  % IN PIXEL                              % std of gaussian kernel (size of neuron) 
...
if refine_components
    [Ain,Cin,center] = manually_refine_components(timeSeries,Ain,Cin,center,Cn,tau,options);
end

%% Save masks
nframes = size(timeSeries,3);
...
%% classify components
[ROIvars.rval_space,ROIvars.rval_time,ROIvars.max_pr,ROIvars.sizeA,keep] = classify_components(timeSeries,A,C,b,f,YrA,options);
A_keep = A(:,keep);
C_keep = C(keep,:);
