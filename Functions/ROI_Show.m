...
if hide_roi
    ...
    % Loading masks
    load(sprintf('masks_%d.mat',maskFileNo));
    NMasks = length(masks);
    ...
    if(isfield(imageData,'AV1'))
        ...
        AV = squeeze(sum(imageData.images_aligned,3))/nframes; % The average image for ch1
        Image_max = max( imageData.images_aligned,[],3 ) ;
    end
